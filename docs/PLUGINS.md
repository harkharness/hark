...
1. **The contract and the Claude Code plugin become public repositories** under
   [@harkharness](https://github.com/harkharness). They are already separate
   compilation units inside Hark, which is what makes the split mechanical
...
There is no timeline promise on the first point. The order is deliberate,
though: the contract goes public before the cockpit does, because the contract
is the part other people need in order to build on it.
...
the list. The contract was shaped by Claude Code first and by six ACP agents
