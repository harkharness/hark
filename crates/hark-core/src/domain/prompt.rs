...
/// Everything Hark knows at question time. Assembled by adapters, consumed here.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Snapshot {
    ...
    /// Recent Hark Q&A (journal tail of the resolved context), oldest first.
...
fn board_section(board: &str) -> String {
    ...
    format!(
        "Quadro de tarefas do usuario (mantido pelo Hark; atualize via campo 'board' da resposta):\n{board}"
    )
}
...
fn workers_section(workers: &[WorkerRecord]) -> String {
    ...
    let lines: Vec<String> = workers
        ...
        .collect();
    format!(
        "Tarefas despachadas pelo Hark (todos os workspaces):\n{}",
        lines.join("\n")
    )
}
