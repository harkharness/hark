...
pub fn significant_terms(text: &str) -> Vec<String> {
    const STOPWORDS: &[&str] = &[
        ...
        "está", "esta", "para", "pra", "por", "que", "sobre", "the", "uma", "vamos", "hark",
    ];
    text.to_lowercase()
...
fn score(terms: &[String], session: &SessionSummary) -> f64 {
    // Whole words on folded tokens: "core" hits "hark-core", never "score".
...
mod tests {
    ...
    fn substring_no_longer_matches_inside_words() {
        ...
        let sessions = vec![session("s2", "hark-core refactor", "refatora o hark-core")];
        assert_eq!(
