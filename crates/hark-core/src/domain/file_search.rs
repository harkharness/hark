...
//! do projeto hark"). No I/O here; callers bring the path list.
...
mod tests {
    ...
    fn every_token_must_match_somewhere() {
        assert!(score("gate rs", "crates/hark-core/src/domain/gate.rs").is_some());
        assert_eq!(score("gate py", "crates/hark-core/src/domain/gate.rs"), None);
    }

    #[test]
    fn queries_are_case_insensitive_and_accept_slashes() {
        assert!(score("Domain/Gate", "crates/hark-core/src/domain/gate.rs").is_some());
