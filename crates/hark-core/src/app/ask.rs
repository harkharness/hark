...
pub struct AskDeps<'a> {
    pub config: &'a Config,
    /// Context chosen via `hark use` (from global state), if any (CLI legacy).
...
/// the context comes from a question hint, else `hark use`, else the default.
pub fn ask(
    question: &str,
    deps: &mut AskDeps,
...
pub fn ask_with_image(
    ...
    let prompt = {
        let soul = std::fs::read_to_string(deps.config.data_dir().join("HARK.md"))
            .unwrap_or_default();
...
/// Where the resolved context keeps its `.hark/`: first repo of the context,
...
/// active (hark use, CLI legacy) > config default > None ("all").
