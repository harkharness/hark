...
pub struct GlobalState {
    /// Context chosen via `hark use`; None means the config default applies.
...
mod tests {
    ...
    fn roundtrips_and_defaults_when_missing() {
        ...
        let state = GlobalState {
            ...
            hark_chat_session: Some("s-chat".into()),
            ..GlobalState::default()
        };
        save(dir.path(), &state).unwrap();
        assert_eq!(load(dir.path()), state);

        // Old state files (no projects/hark_chat keys) must still load.
        std::fs::write(
    ...
    #[test]
    fn state_written_by_the_old_product_name_keeps_the_chat_session() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
            dir.path().join("state.json"),
            r#"{"vox_chat_session":"s-legacy"}"#,
        )
        .unwrap();
        assert_eq!(load(dir.path()).hark_chat_session.as_deref(), Some("s-legacy"));
    }
}
