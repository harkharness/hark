...
//! into a file Vox reads, then runs whatever command was configured before,
...
pub struct BridgePaths {
    ...
    /// Where the wrapper script is written (Vox data dir).
    pub script: PathBuf,
    /// Where the payload lands (Vox data dir).
    pub payload: PathBuf,
...
impl BridgePaths {
    pub fn new(home: &Path, data_dir: &Path) -> Self {
        Self {
            ...
            backup: data_dir.join("settings.json.before-vox"),
        }
    }
}
...
fn script_body(payload: &Path, original: Option<&str>) -> String {
    ...
    format!(
        "#!/bin/sh\n\
         # Written by Vox (statusLine bridge). Delete freely: Vox only reads\n\
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("vox-bridge-{tag}-{}", std::process::id()));
