...
mod tests {
    ...
    fn folds_accents_and_case() {
        ...
        assert_eq!(tokens("vox-core targeting"), vec!["vox", "core", "targeting"]);
    }

    #[test]
    fn matches_whole_words_never_substrings() {
        // "core" must NOT hit "score keeper" (substring), but MUST hit
        // "vox-core" (hyphen splits into whole words).
        let out = rank_titles(
            "core parser",
            vec![("junk", "score keeper encore", "2026-08-19")],
        );
        assert_eq!(out, "none");
        let out = rank_titles("core parser", vec![("hit", "vox-core parser fixes", "2026-08-19")]);
        assert_eq!(out, "hit:hit");
    }

    #[test]
    fn requires_half_the_terms_to_match() {
        // The 19/08 incident: a fresh vox card with one junk "core" hit
        // must NOT win "migração dos serviços Assinaturas Core".
        let out = rank_titles(
            "migração dos serviços Assinaturas Core",
            vec![("vox", "vox-core targeting refactor", "2026-08-19T12:00:00Z")],
        );
        assert_eq!(out, "none");
    }
